...
const RestaurantDetails = () => {
    const restaurants=[
        ...
        var inp='', matched=[]
        const [match,setMatch]=useState(restaurants)
        const clickHandler=()=>{
            inp =document.getElementById('searchInp').value
            for(let i=0;i<restaurants.length;i++){
                if(restaurants[i].name.slice(0,inp.length).toUpperCase()==inp.toUpperCase() || restaurants[i].cuisine_type.slice(0,inp.length).toUpperCase()==inp.toUpperCase() || restaurants[i].neighborhood.slice(0,inp.length).toUpperCase()==inp.toUpperCase()){
