...
const payHandler=(e)=>{
  tot = document.getElementById('tot').innerHTML
  setTot(tot)
  navigate('/payment')
}
  ...
  return(
    <div className="App">
      <Routes>
        ...
        <Route path='/home' element={<MainPage match={match} cart={cart} Cart={cartHandler}  sweets={sweets} div={divHandler} add={addHandler} />}/>
        <Route path='/sweetPage' element={<SweetPage cart={cart} Cart={cartHandler} match={newAr} add={addHandler}/>}/>
        <Route path='/cart' element={<Cart arr={cart} minus={minusHandler} plus={plusHandler} pay={payHandler} />}/>
        <Route path='/payment' element={<Payment tot={tot}/>}/>
      </Routes>
    </div>
  )
}
